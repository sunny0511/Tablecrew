...
| Won't (v1) | Recurring Table series (auto-generated weekly instances) | A single host action spawning a repeating series of Tables automatically. | Superseded by the more general Crew recurring-ritual nudge system planned for Phase 2, which solves the same need without a separate scheduling subsystem. |
