...
1. FR-T1–T33 (including FR-T9a, FR-T14a, FR-T14b, FR-T14c, FR-T29a) and FR-C1–C5 are implemented and pass `TESTING.md` acceptance suites, **except FR-T22–FR-T25a (bill-splitting)** and **FR-T14c (recurring Crews)**, which are Phase 1 requirements, not Phase 0 launch blockers (Discover requirements FR-D1–D16 are likewise Phase 1 launch requirements, not Phase 0 launch blockers). *Corrected 2026-08: bill-splitting was previously bundled into this Phase 0 launch-blocker list, contradicting the founder-confirmed build order in `TASKS.md`, which sequences payments after Discover. Moved to align with `ROADMAP.md`'s corrected Phase 1 feature list — see `CHANGELOG.md`.* **Build-vs-launch note, added 2026-08:** FR-D1–D16 are Phase 1 *launch* requirements (unchanged) but are now *built* during the Foundation engineering effort per `docs/IMPLEMENTATION_PLAN.md`'s new Milestone F7 — "not launch blockers for Phase 0" describes when they must work for real users, not when the code gets written. Same distinction applies to FR-T14c (recurring Crews), which is a real Phase 1 requirement, not a cancelled one — see `docs/FEATURES.md`.
