...
- **`tables/{id}.location.venueNameSnapshot`:** avoids a `venues/{id}` read on every Table render just to show the venue's name; refreshed via trigger on `venues/{id}.name` change (rare).
