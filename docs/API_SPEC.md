...
**Rewritten 2026-08 (ADR [0007](adr/0007-manual-identity-verification-for-phase-0.md), superseding ADR 0005 for Phase 0).** This section previously specified a single `completeIdentityVerification` callable that took a `personaInquiryId` and server-side-fetched the result from Persona's API. Tier 2 is now performed by **manual human review** with no identity vendor in the flow, so that contract is replaced by the two callables below: one the user calls to submit, one an administrator calls to decide. The security property that mattered most in the old contract is preserved exactly — **the client never supplies a pass/fail signal, and no client can self-elevate its own verification tier** — it is now enforced by an admin-claim check rather than by a server-side vendor lookup. See ADR 0007 for what this trade costs (weaker liveness, no cross-account ban-evasion detection, TableCrew now custodian of ID images) and why it was accepted for Phase 0.

#### `submitIdentityVerification`

Records a user's Tier 2 submission for human review (`SCREEN_SPECIFICATIONS.md` Screen 8). The ID and selfie images are uploaded by the client directly to Cloud Storage under a path only that user may write and **no client may read** (`SECURITY.md`, `FIREBASE.md` §2.5a) before this callable is invoked; this callable receives only the upload identifiers, never image bytes.

- **Request:** `{ idDocumentUploadId: string, selfieUploadId: string, documentType: "aadhaar_offline" | "passport" | "drivers_license" | "voter_id" | "other" }` — `documentType` is a reviewer aid, self-declared and never trusted as verification of anything; the reviewer confirms the actual document type by looking at it.
- **Response:** `{ submissionId: string, status: "pending_review" }`
- **Errors:** `failed-precondition` (`ALREADY_VERIFIED` — a no-op success shape for a caller already `id_verified`, matching the old contract's treatment of the same case), `failed-precondition` (`REVIEW_ALREADY_PENDING` — one open submission per user at a time; re-submitting while a review is outstanding is rejected rather than queuing a second one for the same reviewer), `not-found` (`UPLOAD_NOT_FOUND` — one or both upload ids do not resolve to an object under the caller's own Storage prefix), `resource-exhausted` (`RATE_LIMITED`).
- **Server behavior:** confirms via the Admin SDK that both referenced objects actually exist under `identity-verifications/{uid}/` — a client cannot submit a reference to another user's upload, since the prefix is derived from `context.auth.uid` server-side rather than taken from the request. Creates `identityVerifications/{submissionId}` (`DATABASE.md` §3.10) with `status: "pending_review"`. **Writes nothing to `users/{uid}/private/profile.verification`** — submission is not verification, and the tier is only ever written by the review callable below.
- **Abuse prevention:** rate-limited to **3 submissions per rolling 24h per uid**, reusing the retry cap `SCREEN_SPECIFICATIONS.md` Screen 8 already specifies rather than inventing a second, different number. The cap matters more under manual review than it did under a vendor flow, because each submission consumes human reviewer time rather than vendor compute.

#### `reviewIdentityVerification` **[admin-only]**

Applies a human reviewer's decision to a pending submission. This is the only endpoint in the API surface that grants `verificationTier: "id_verified"`, and the only one gated on administrator authority rather than ordinary user authentication.

- **Request:** `{ submissionId: string, decision: "approve" | "reject", dobMatchesId: boolean, rejectionReason?: string }`
- **Response:** `{ submissionId: string, status: "approved" | "rejected", verificationTier: "id_verified" | "phone_verified" | "unverified" }`
- **Errors:** `permission-denied` (`ADMIN_ONLY` — caller lacks the `admin` custom claim), `not-found` (no such submission), `failed-precondition` (`ALREADY_REVIEWED` — a submission carries exactly one terminal decision; re-deciding requires a new submission), `invalid-argument` (`REJECTION_REASON_REQUIRED` on a reject with no reason; `DOB_ATTESTATION_REQUIRED` on an approve with `dobMatchesId: false` — see below), `failed-precondition` (`ACCOUNT_UNDER_REVIEW`).
- **Server behavior:** verifies the caller's `admin` custom claim server-side (a claim, never a Firestore field a compromised client could write). On approve, three things must hold before the tier is granted:
  1. **`dobMatchesId` must be `true`.** `SECURITY.md` requires the ID-derived date of birth to be cross-checked against the self-reported DOB from Screen 4 before Tier 2 is granted. No OCR runs under ADR 0007, so this is an explicit **reviewer attestation** — the reviewer confirms they compared the document against `private/profile.dateOfBirth`. An approve carrying `dobMatchesId: false` is rejected as `invalid-argument` rather than quietly granting the tier, so the check cannot be skipped by omission.
  2. **The account must not be under active Trust & Safety review.** `trustSignals` and open reports are re-checked **at the moment the grant is applied**, not at submission time — the ordering guarantee `SECURITY.md` already requires so that a report filed mid-verification is not outrun by the verification completing. If an open report exists, the decision records as held (`ACCOUNT_UNDER_REVIEW`) rather than granting.
  3. Only then does it write `users/{uid}/private/profile.verification` (`verificationTier: "id_verified"`, `idVerified: true`, `verifiedAt`) via the Admin SDK, and, in the same batch, `users/{uid}.verificationTierPublic` — the identical write path the superseded Persona contract used, deliberately unchanged so a future vendor ADR can restore an automated decision source without touching it.

  **Correction, 2026-08, found while implementing this:** the superseded text here said the public badge is mirrored by "the `DATABASE.md` §4 denormalization trigger." No such trigger exists. `functions/src/triggers/` contains exactly one trigger — the Table→Typesense sync added earlier in Milestone F7 — and `DATABASE.md` §4 describes `verificationTierPublic` as trigger-maintained in the abstract without any milestone ever having scheduled building it. `completeAccountSetup` has always written this field directly for the same reason. Both documents are therefore written explicitly by this callable, in one batch, rather than one write being left to fire a mirror that was never built. This is the same shape of gap Milestone F5 found for photo moderation (described in `FIREBASE.md` since F0, scheduled by no milestone) — worth noting because it means any *other* field `DATABASE.md` §4 describes as trigger-maintained deserves the same check before being relied on.

  On **either** outcome, both Storage objects are deleted immediately (`DATABASE.md` §3.10, ADR 0007's data-minimization consequence) and the submission document retains only the decision record — reviewer uid, timestamp, decision, reason — never ID-derived content. A rejection leaves the tier at its existing value and does not lock the account; the user may re-submit, subject to the rate limit above.
- **Abuse prevention:** the admin-claim gate is the primary control. Every decision persists an immutable audit record naming the reviewing uid, which survives the image deletion — the accountability trail for a manual process where a single person can grant Discover access.
...
- Identity-verification tiering and re-verification policy underlying `submitIdentityVerification`/`reviewIdentityVerification` (§3.7): `SECURITY.md` §Identity Verification Tiers. (Endpoint names corrected 2026-08 — this line still cited the `completeIdentityVerification` contract that ADR 0007 superseded.)
