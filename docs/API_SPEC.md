...
- **Additive changes** (new optional request fields, new response fields, new error codes) ship without a version bump — every callable's request/response Zod schema treats new fields as optional with defaults, and clients on older app versions simply don't send/read the new fields. This is the overwhelming majority of API evolution.
