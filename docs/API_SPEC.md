...
- **Server behavior:** verifies the caller's `admin` custom claim server-side (a claim, never a Firestore field a compromised client could write). On approve, three things must hold before the tier is granted:
  ...
  3. Only then does it write `users/{uid}/private/profile.verification` (`verificationTier: "id_verified"`, `idVerified: true`, `verifiedAt`) via the Admin SDK, which the `DATABASE.md` §4 denormalization trigger mirrors to the public `verificationTierPublic` badge — the identical write path the superseded Persona contract used, deliberately unchanged so a future vendor ADR can restore an automated decision source without touching it.
