...
**Note on build timing vs. launch timing (added 2026-08).** This roadmap's phases describe when a feature goes **live for real users** — that hasn't changed. Separately, `docs/IMPLEMENTATION_PLAN.md`'s engineering milestone plan now **builds** Discover (identity verification, Typesense search, Open Tables) during the Foundation engineering effort that delivers Phase 0, immediately after Trust & Safety infrastructure is live, rather than waiting until Phase 1's engineering work begins. This is a deliberate founder call to validate Discover — TableCrew's most differentiated, least commodity-replicable feature — as early as practically possible once the safety backbone this section already argues for is in place, not a reversal of the Crew-first safety rationale above. Discover's code ships behind a Remote Config flag defaulted off (`docs/DEPLOYMENT.md`'s Feature Flag Lifecycle) through all of Phase 0 — the Phase 0 success gate below and the Phase 1 entry criteria are unchanged, including the legal/statute review this document already requires before Discover goes live to anyone. Read every "shipped"/"introduced" below as "live to real users," not "exists in the codebase."
